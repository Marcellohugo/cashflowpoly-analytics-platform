...
// Use the browser timezone consistently for static timestamps and live session events.
window.cashflowpolyFormatDateTime = (rawValue) => {
  const date = new Date(rawValue);
  if (!rawValue || Number.isNaN(date.getTime())) return "-";
  return date.toLocaleString(document.documentElement.lang === "en" ? "en-US" : "id-ID", {
    day: "2-digit", month: "short", year: "numeric",
    hour: "2-digit", minute: "2-digit", second: "2-digit", timeZoneName: "short"
  });
};
document.querySelectorAll('time[data-local-time]').forEach(time => {
  time.textContent = window.cashflowpolyFormatDateTime(time.dateTime);
});
