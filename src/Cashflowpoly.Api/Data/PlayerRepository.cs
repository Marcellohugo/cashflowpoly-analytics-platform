...
public sealed class PlayerRepository
{
    ...
    public async Task<List<SessionRosterPlayerDb>> ListSessionRostersAsync(Guid[] sessionIds, CancellationToken ct)
    {
        const string sql = """
            select sp.session_id, sp.user_id,
                   coalesce(nullif(sp.player_name, ''), u.display_name) as display_name,
                   sp.player_order_no as player_order,
                   fs.rank_no as final_rank, fs.total_points as happiness_points_total
            from session_participants sp
            join app_users u on u.user_id = sp.user_id
            left join session_final_scores fs on fs.session_id = sp.session_id
                and fs.session_participant_id = sp.session_participant_id
            where sp.session_id = any(@sessionIds)
            order by sp.session_id, sp.player_order_no, sp.joined_at, sp.user_id
            """;
        await using var conn = await _dataSource.OpenConnectionAsync(ct);
        return (await conn.QueryAsync<SessionRosterPlayerDb>(
...
public sealed class SessionRosterPlayerDb
{
