...
{
    Task<(SessionRostersResponse? Result, int StatusCode, ErrorResponse? Error)> GetSessionRostersAsync(
        bool includeResults, ClaimsPrincipal user, CancellationToken ct);

    Task<(PlayerGameplayHistoryResponse? Result, int StatusCode, ErrorResponse? Error)> GetPlayerGameplayHistoryAsync(
